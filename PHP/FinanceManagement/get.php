...
function getRequest($table, $key){
  ...
  switch ($table) {
    ...
    case 'payments': $sql ="select * from payments".($key?" WHERE ID=$key AND Rollback=0":" WHERE Rollback=0")." AND `Date` > (ADDDATE(CURDATE(), INTERVAL -3 MONTH)) order by ID desc"; break;
